...
test("updates Memory title, Original Story and optional Stage without touching its exhibition data", () => {
  ...
  try {
    ...
    database.prepare(
      "INSERT INTO later_notes (id, memory_id, content, created_at) VALUES (?, ?, ?, ?)"
    ...
    const assigned = database.prepare(
      "SELECT title, story, stage_id FROM memories WHERE id = ?"
    ...
    const unassigned = database.prepare(
      "SELECT stage_id FROM memories WHERE id = ?"
    ).get("memory-edit") as { stage_id: string | null };
    const noteCount = database.prepare(
      "SELECT COUNT(*) AS count FROM later_notes WHERE memory_id = ?"
    ).get("memory-edit") as { count: number };
    assert.equal(unassigned.stage_id, null);
    assert.equal(noteCount.count, 1);

    assert.throws(() => updateMemoryDetailsInDatabase(activeDatabase, "memory-edit", {
      title: "",
      story: "Story",
      stageId: null,
    }), /TITLE_REQUIRED/);
