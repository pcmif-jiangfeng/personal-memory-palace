...
export function ownerCookie() { return { name: cookieName, value: digest(process.env.MEMORY_PALACE_OWNER_PASSWORD ?? ""), httpOnly: true, sameSite: "lax" as const, secure: process.env.NODE_ENV === "production", path: "/", maxAge: 60 * 60 * 24 * 30 }; }
export { cookieName };
